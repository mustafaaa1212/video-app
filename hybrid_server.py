...
# Email configuration
SENDER_EMAIL = "sadikot5580@gmail.com"
...
def process_video_task(video_path, interval, video_filename, process_id):
    ...
    try:
        ...
        # Send email
        email_success = send_email(zip_path, video_filename, interval, screenshot_count)
        
        # Store result for download (keep file for download)
        ...
        if email_success:
            logging.info(f"Video processing completed successfully - {screenshot_count} screenshots, email sent")
        else:
            logging.info(f"Video processing completed - {screenshot_count} screenshots, email failed")
...
@app.route('/hybrid_script.js')
def hybrid_script():
    return send_from_directory('.', 'hybrid_script.js')
...
def upload_file():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            return jsonify({
                'success': True, 
                'message': 'Video uploaded successfully! Processing screenshots and sending email...',
